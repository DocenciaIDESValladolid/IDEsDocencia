// Javi -> el on sirve para eventos, es decir, cuando se vaya a la página de inicio y tenga el id = "mappage"
// El # indica un identificador pero s epuede poner $(document) o $("p") para un párrafo
// El . indica una clase
// el .on sirve para que se pueda realizar varios funciones
...
// Javi -> Primero se pondrá un <div id="apptst"> Click here </div> y ese identificador es el que
// luego se pone después de $ para saber a que función invocar, en este caso se invoca a  tst()
...
async function tst(){
    ...
    // Javi -> utiliza la API de OpenLayers para obtener primero la posición en la que nos encontramos y luego obtiene
    // la geometría (el punto) que haya marcado el usuario
    ...
    //Javi -> el toast son las ventanas de android
    ...
    //Javi -> obtiene las coordenadas de la feature de destino que ha marcado el usuario
    ...
    //Javi -> hace exactamente lo mismo pero con las coordenadas del punto de destino
	var destino=new ol.geom.Point([aux2[0],aux2[1]]);
    //Javi -> inicializa una geometría que será un punto con un destino y las coordenadas en x = 0 e y = 0
    ...
    //Javi -> crea variables para hacer el algoritmo del cálculo de la distancia
	var contador = 0;
	var llegada=0;
	var ptosIntermedios = new Array(6);
	
    //Javi -> Supongo que como máximo quieren 5 saltos
...
function calculoDistancia2(punto1,punto2){
	//Javi -> Simplemente obtiene las coordenadas del punto1, las coordenadas del punto 2 y calcula la distancia.
    var point1= punto1.getCoordinates();
	var point2= punto2.getCoordinates();
    
    //Javi -> Realiza el cálculo de distancia entre dos vectores -> distancia = raiz ((x1-x2)^2 + (y1-y2)^2)
	...
	//Javi -> pone el promise porque es una función asíncrona que está siendo espera por el await
...
*/
//Javi -> Esta función se supone que no la vamos a necesitar porque ya sabremos el punto más cercano con vec:snap
...
*/
//Javi -> esta función es interesante para saber como hacer una jquery a un servidor WFS
function obtenerPtosRecargaMunicipio(){ 
      ...
      //Javi -> IMPORTANTÍSIMO para mandar la petición al WFS
...
//Javi -> IMPORTANTE PARA SABER HACER UNA PETICIÓN WFS
