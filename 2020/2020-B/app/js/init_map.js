function initmap() {
  ...
  openStreetMapGeocoder = GeocoderJS.createGeocoder('openstreetmap');
    ...
    //NACHO
    fuenteOscuridad = new ol.source.Vector();
	var vectorOscuridad = new ol.layer.Vector({
		  name: 'Zonas oscuras',
		  source: fuenteOscuridad,
        });
        ...
        var img = new Image();
        img.src = 'https://i.pinimg.com/originals/2f/77/88/2f77880be2f62ae3b727b293b9cd4568.jpg';
        
        img.onload = function(){
          var pattern = ctx.createPattern(img, 'repeat');*/
          
          vectorOscuridad.setStyle(new ol.style.Style({
            fill: new ol.style.Fill({
              color: 'rgba(0, 5, 71, 0.6)',
              opacity: 0.5
            })
          }));
       // }

    fuenteRecomendable = new ol.source.Vector();
	var vectorRecomendable = new ol.layer.Vector({
		  name: 'Zonas recomendables',
		  source: fuenteRecomendable,
        });
        ...
        var img2 = new Image();
        img2.src = 'https://image.freepik.com/free-photo/abstract-pattern-gradient-wallpaper_41691-426.jpg';

        


        img2.onload = function(){
          var pattern2 = ctx2.createPattern(img2, 'repeat');
          
          vectorRecomendable.setStyle(new ol.style.Style({
            fill: new ol.style.Fill({
              color: pattern2
            }),
            stroke: new ol.style.Stroke({
                color: 'rgba(50, 1, 1, 0.9)', width: 1
              })
          }));
        }
    ...
    fuentePunto = new ol.source.Vector();
    var vectorPunto = new ol.layer.Vector({
            name: 'Punto recomendado',
            source: fuentePunto,
        });


        vectorPunto.setStyle(new ol.style.Style({
            image: new ol.style.Circle({
         radius: 7,
    ...
    fuenteRuta = new ol.source.Vector();
    var vectorRuta = new ol.layer.Vector({
            name: 'Ruta al punto recomendado',
            source: fuenteRuta,
        });

        vectorRuta.setStyle(new ol.style.Style({
            fill: new ol.style.Fill({color: 'rgba(235, 172, 0, 1)'}),
            stroke: new ol.style.Stroke({
            color: 'rgba(235, 172, 0, 1)', width: 5,
            opacity: 0.4
            })
        }));
    ...
    aeriallayer.set("name", "Mapa satélite");
    var roadlayer = new ol.layer.Tile({
        source: new ol.source.OSM()
    });
    roadlayer.set("name", "Mapa de carreteras");
    ...
    layers = [layergroup, userPosition, markerVector, vectorOscuridad, vectorRecomendable, vectorPunto, vectorRuta];
    // New Custom zoom.
    ...
    add_layer_to_list(vectorOscuridad);
    add_layer_to_list(vectorRecomendable);
    add_layer_to_list(vectorPunto);
    add_layer_to_list(vectorRuta);
    ...
    map.on('click', function (evt) {
        ...
        if (!hasFeature) {
            selectedCoordinates = map.getEventCoordinate(evt.originalEvent);
            markerFeature.setGeometry(selectedCoordinates ?
                new ol.geom.Point(selectedCoordinates) : null);
        }
    });
}
/*-------------------------------Functions-----------------------------------*/
